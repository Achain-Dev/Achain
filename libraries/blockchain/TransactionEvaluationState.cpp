    ...
    namespace blockchain {
        ...
        void TransactionEvaluationState::post_evaluate()
        {
            try {
                ...
                /* if( _current_state->get_head_block_num() >= ACT_V0_6_4_FORK_BLOCK_NUM )
                {
                for( const auto& op : trx.operations )
                {
                if( operation_type_enum( op.type ) == cover_op_type )
                {
                FC_ASSERT( balance[ 0 ] <= ACT_BLOCKCHAIN_PRECISION );
                }
                }
                }*/
            } FC_CAPTURE_AND_RETHROW()
        ...
        void TransactionEvaluationState::evaluate(const SignedTransaction& trx_arg, bool ignore_state)
        {
            ...
            try {
                ...
                try {
                    ...
                    if ((_current_state->now() + ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC + ACT_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration)
                        FC_CAPTURE_AND_THROW(invalid_transaction_expiration, (trx_arg)(_current_state->now()));

                    if (origin_trx_basic_verify(trx) && (trx.data_size() > ACT_BLOCKCHAIN_MAX_TRX_SIZE))
                    {
					...
					if (num_of_signature > ACT_BLOCKCHAIN_MAX_SIGNAGTURE_NUM)
					{
						FC_CAPTURE_AND_THROW(too_much_signature, (ACT_BLOCKCHAIN_MAX_SIGNAGTURE_NUM)(num_of_signature));
					}
					...
					int signum_to_charge = num_of_signature - ACT_BLOCKCHAIN_FREESIGNATURE_LIMIT;
					if (signum_to_charge>=0)
					{
						required_fees.amount += signum_to_charge*ACT_BLOCKCHAIN_EXTRA_SIGNATURE_FEE;
					}
        ...
        SignedTransaction TransactionEvaluationState::sandbox_evaluate(const SignedTransaction &trx_arg, bool& ignore_check_required_fee)
        {
            try
            {
                ...
                try
                {
                    ...
                    if ((_current_state->now() + ACT_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration)
                        FC_CAPTURE_AND_THROW(invalid_transaction_expiration, (trx_arg)(_current_state->now()));
