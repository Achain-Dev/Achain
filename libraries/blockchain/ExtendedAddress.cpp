    ...
    namespace blockchain {
        ...
        ExtendedAddress::ExtendedAddress(const std::string& base58str)
        {
            try {
                ...
                uint32_t checksum = 0;
                std::string prefix(ACT_ADDRESS_PREFIX);
                const size_t prefix_len = prefix.size();
                FC_ASSERT(base58str.size() > prefix_len, "Address must be longer than ACT_ADDRESS_PREFIX");
                FC_ASSERT(base58str.substr(0, prefix_len) == prefix, "ACT_ADDRESS_PREFIX must be in front of the address");
        ...
        ExtendedAddress::operator std::string()const
        {
            ...
            return ACT_ADDRESS_PREFIX + fc::to_base58(data.data(), data.size());
        }
