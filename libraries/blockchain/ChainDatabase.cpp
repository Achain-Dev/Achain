...
namespace thinkyoung {
    namespace blockchain {
        ...
        namespace detail
        {
            void ChainDatabaseImpl::revalidate_pending()
            {
                ...
                while (itr.valid())
                {
                    ...
                    try
                    {
                        ...
                        count++;
                        if (count > ACT_BLOCKCHAIN_REVALIDATE_MAX_TRX_COUNT)
                            break;
            ...
            bool ChainDatabaseImpl::replay_required(const fc::path& data_dir)
            {
                try {
                    ...
                    return !entry.valid() || entry->value.as_uint64() != ACT_BLOCKCHAIN_DATABASE_VERSION;
                } FC_CAPTURE_AND_RETHROW((data_dir))
            }
            ...
            void ChainDatabaseImpl::open_database(const fc::path& data_dir)
            {
                try {
                    ...
                    _act_input_balance_entry.open(data_dir / "index/_act_input_balance_entry");
                    _act_full_entry.open(data_dir / "index/_act_full_entry");
            ...
            DigestType ChainDatabaseImpl::initialize_genesis(const optional<path>& genesis_file, const bool statistics_enabled)
            {
                try {
                    ...
                    // Check genesis state
                    FC_ASSERT(config.delegates.size() >= ACT_BLOCKCHAIN_NUM_DELEGATES,
                        "genesis.json does not contain enough initial delegates!",
                        ("required", ACT_BLOCKCHAIN_NUM_DELEGATES)("provided", config.delegates.size()));
                    ...
                    base_asset.symbol = ACT_BLOCKCHAIN_SYMBOL;
                    base_asset.name = ACT_BLOCKCHAIN_NAME;
                    base_asset.description = ACT_BLOCKCHAIN_DESCRIPTION;
                    base_asset.public_data = variant("");
                    base_asset.issuer_account_id = 0;
                    base_asset.precision = ACT_BLOCKCHAIN_PRECISION;
                    base_asset.registration_date = timestamp;
                    base_asset.last_update = timestamp;
                    base_asset.current_share_supply = total_base_supply;
                    base_asset.maximum_share_supply = ACT_BLOCKCHAIN_MAX_SHARES;
                    ...
                    for (const GenesisAsset& asset : config.market_assets)
                    {
                        ...
                        rec.maximum_share_supply = ACT_BLOCKCHAIN_MAX_SHARES;
                ...
                const PendingChainStatePtr& pending_state)
            {
                try {
                    ...
                    for (const auto& trx : block_data.user_transactions)
                    {
                        ...
                        FC_ASSERT(trx.data_size() <= ACT_BLOCKCHAIN_MAX_TRX_SIZE, "trx size is out of the max trx size range");
            ...
            {
                try {
                    // if( pending_state->get_head_block_num() < ACT_V0_6_0_FORK_BLOCK_NUM )
                    ...
                    static const uint32_t blocks_per_two_weeks = 14 * ACT_BLOCKCHAIN_BLOCKS_PER_DAY;
                ...
                const PendingChainStatePtr& pending_state)
            {
                try {
                    ...
                    if (block_num > ACT_BLOCKCHAIN_MAX_UNDO_HISTORY)
                    {
                        const uint32_t old_block_num = block_num - ACT_BLOCKCHAIN_MAX_UNDO_HISTORY;
            ...
            void ChainDatabaseImpl::verify_header(const DigestBlock& block_digest, const PublicKeyType& block_signee)const
            {
                try {
                    ...
                    if (block_digest.timestamp.sec_since_epoch() % ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC != 0)
                    ...
                    if (block_digest.timestamp > (now + ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2))
            ...
            /**
             *  A block should be produced every ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC. If we do not have a
                ...
                const PendingChainStatePtr& pending_state)const
            {
                try {
                    ...
                    if (head_block.block_num > 0) block_timestamp = head_block.timestamp + ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    ...
                    for (; block_timestamp < block_header.timestamp;
                        block_timestamp += ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
                    ...
                    if (required_confirmations > ACT_BLOCKCHAIN_NUM_DELEGATES * 3)
                        required_confirmations = 3 * ACT_BLOCKCHAIN_NUM_DELEGATES;
            ...
            {
                try {//todo remove????
                    // if( pending_state->get_head_block_num() < ACT_V0_7_0_FORK_BLOCK_NUM )
                    // return update_active_delegate_list_v1( block_num, pending_state );

                    if (block_num % ACT_BLOCKCHAIN_NUM_DELEGATES != 0)
                        return;
			...
			*/
            ...
            void ChainDatabaseImpl::extend_chain(const FullBlock& block_data)
            {
                try {
                    ...
                    try
                    {
                        ...
                        if (thinkyoung::client::g_client->get_wallet() != nullptr&&thinkyoung::client::g_client->get_wallet()->is_open())
                        {
                            if (!thinkyoung::client::g_client->get_wallet()->get_my_delegates(thinkyoung::wallet::enabled_delegate_status).empty())
                            {
                                ...
                                _relay_fee = fee > ACT_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee : ACT_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                            }
                            else
                            {
                                auto fee = thinkyoung::client::g_client->get_wallet()->get_transaction_fee();
                                _relay_fee = fee.amount > ACT_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee.amount : ACT_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                                //_relay_fee = ACT_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                            }
                        }
                    }
                    catch (const fc::exception& e)
                    ...
                    if ((now() - block_data.timestamp).to_seconds() < ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
        ...
        {
            return get_delegates_by_vote(0, ACT_BLOCKCHAIN_NUM_DELEGATES);
        }
            ...
            try {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        my->open_database(data_dir);
                        store_property_entry(PropertyIdType::database_version, variant(ACT_BLOCKCHAIN_DATABASE_VERSION));
                        ...
                        const auto insert_block = [&](const FullBlock& block)
                        {
                            if (blocks_indexed % 200 == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    progress = float(blocks_indexed * ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC) / seconds;
                                }
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                if (last_known_block_num > ACT_BLOCKCHAIN_MAX_UNDO_HISTORY)
                                    my->_min_undo_block = last_known_block_num - ACT_BLOCKCHAIN_MAX_UNDO_HISTORY;
        ...
        void ChainDatabase::close()
        {
            try {
                ...
                my->_act_input_balance_entry.close();
                my->_act_full_entry.close();
            ...
            const std::vector<AccountIdType>& ordered_delegates)const
        {
            try {
                ...
                auto delegate_pos = slot_number % ACT_BLOCKCHAIN_NUM_DELEGATES;
        ...
        optional<time_point_sec> ChainDatabase::get_next_producible_block_timestamp(const vector<AccountIdType>& delegate_ids)const
        {
            try {
                ...
                if (next_block_time <= now()) next_block_time += ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                auto last_block_time = next_block_time + (ACT_BLOCKCHAIN_NUM_DELEGATES * ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC);

                auto active_delegates = get_active_delegates();
                for (; next_block_time < last_block_time; next_block_time += ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
                {
        ...
        BlockForkData ChainDatabase::push_block(const FullBlock& block_data)
        {
            try {
                ...
                if (head_block_num > ACT_BLOCKCHAIN_MAX_UNDO_HISTORY &&
                    block_data.block_num <= (head_block_num - ACT_BLOCKCHAIN_MAX_UNDO_HISTORY))
                {
                    elog("block ${new_block_hash} (number ${new_block_num}) is on a fork older than "
                        ...
                        ("head_block_num", head_block_num)("undo_history", ACT_BLOCKCHAIN_MAX_UNDO_HISTORY));
        ...
        */
        double ChainDatabase::get_average_delegate_participation()const
        {
            try {
                ...
                else if (head_num <= ACT_BLOCKCHAIN_NUM_DELEGATES)
                {
                    // what percent of the maximum total blocks that could have been produced
                    // have been produced.
                    const auto expected_blocks = (now - get_block_header(1).timestamp).to_seconds() / ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                ...
                else
                {
                    ...
                    const auto starting_time = get_block_header(head_num - ACT_BLOCKCHAIN_NUM_DELEGATES).timestamp;
                    const auto expected_production = (now - starting_time).to_seconds() / ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    return 100 * double(ACT_BLOCKCHAIN_NUM_DELEGATES) / expected_production;
                }
            } FC_RETHROW_EXCEPTIONS(warn, "")
        }
        ...
        TransactionEvaluationStatePtr ChainDatabase::store_pending_transaction(const SignedTransaction& trx, bool override_limits, bool contract_vm_exec, bool cache)
        {
            try {
                ...
                if (!override_limits)
                {
                    if (my->_pending_fee_index.size() > ACT_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE)
                    {
                        auto overage = my->_pending_fee_index.size() - ACT_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
        ...
        FullBlock ChainDatabase::generate_block(const time_point_sec block_timestamp, const DelegateConfig& config)
        {
            try {
                ...
                //if( pending_state->get_head_block_num() >= ACT_V0_4_9_FORK_BLOCK_NUM )
                        ...
                        try
                        {
                            ...
                            auto pending_trx_state = origin_trx_state;
                            {
                                ...
                                if (trx_eval_state->p_result_trx.operations.size() > 0)
                                {
                                    ...
                                    if (iMessageLength > ACT_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
                                    {
                                        imessage_fee = config.transaction_imessage_min_fee_coe * (iMessageLength - ACT_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
                                    }
                ...
                // if( new_block.block_num < ACT_V0_7_0_FORK_BLOCK_NUM )
        ...
        string ChainDatabase::export_fork_graph(uint32_t start_block, uint32_t end_block, const fc::path& filename)const
        {
            ...
            for (auto block_itr = my->_block_id_to_full_block.begin(); block_itr.valid(); ++block_itr)
            {
                ...
                {
                    unsigned rank = (unsigned)((block.timestamp - start_time).to_seconds() / ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
        ...
        vector<ActTrxidBalance> ChainDatabase::fetch_act_input_balance(const uint32_t & block_num)
        {
            try{
                vector < ActTrxidBalance > results;
                for (auto iter = my->_act_input_balance_entry.unordered_begin();
                    iter != my->_act_input_balance_entry.unordered_end(); ++iter)
                {
                    ActTrxidBalance actTemp;
                    actTemp.block_num = block_num + 1;
					set<ActTrxidBalance>::iterator block_iter = iter->second.lower_bound(actTemp);
        ...
        vector<ActTrxidBalance> ChainDatabase::fetch_act_full_entry(const uint32_t& block_num, const uint32_t & last_scan_block_num)
        {
            try{
                vector < ActTrxidBalance > results;
                ...
                for (auto iter = my->_act_full_entry.unordered_begin();
                    iter != my->_act_full_entry.unordered_end(); ++iter)
                {
                    auto block_iter = iter->second.act_block_sort.lower_bound(block_num + 1);
                    for (; block_iter != iter->second.act_block_sort.end(); block_iter++)
                    {
        ...
        void ChainDatabase::transaction_insert_to_act_full_entry(const string & act_account, const ActTrxidBalance & act_balance_record)
        {
            auto iter_entry = my->_act_full_entry.unordered_find(act_account);
            ActBalanceEntry actentrys;
            if (iter_entry != my->_act_full_entry.unordered_end())
            {
                actentrys = iter_entry->second;
                if (actentrys.act_trxid_sort.count(act_balance_record.trx_id) != 0)
                {
                    return;
                }
            }
            actentrys.act_trxid_sort.insert(act_balance_record.trx_id);
            actentrys.act_block_sort.insert(std::make_pair(act_balance_record.block_num, act_balance_record));
            my->_act_full_entry.store(act_account, actentrys);
        }
        void ChainDatabase::transaction_erase_from_act_full_entry(const string & act_account, const ActTrxidBalance & act_balance_record)
        {
            auto iter_entry = my->_act_full_entry.unordered_find(act_account);
            if (iter_entry != my->_act_full_entry.unordered_end())
            {
                ActBalanceEntry actentrys = iter_entry->second;
                actentrys.act_trxid_sort.erase(act_balance_record.trx_id);
                auto iter = actentrys.act_block_sort.lower_bound(act_balance_record.block_num);
                for (; iter != actentrys.act_block_sort.end();)
                {
                    if (iter->second.block_num != act_balance_record.block_num)
                    {
                        break;
                    }
                    if (iter->second.trx_id == act_balance_record.trx_id)
                    {
                        iter = actentrys.act_block_sort.erase(iter);
                    }
                    else
                    {
                        ++iter;
                ...
                if (!(actentrys.act_block_sort.empty()))
                {
                    my->_act_full_entry.store(act_account, actentrys);
                }
                else
                {
                    my->_act_full_entry.remove(act_account);
                }
            }
        }
        void ChainDatabase::transaction_insert_to_act_balance(const string & act_account, const ActTrxidBalance & act_balance_entry)
        {
            auto iter_ids = my->_act_input_balance_entry.unordered_find(act_account);
            set<ActTrxidBalance> ids;
            if (iter_ids != my->_act_input_balance_entry.unordered_end())
            {
                ids = iter_ids->second;
            }
            ids.insert(act_balance_entry);
            my->_act_input_balance_entry.store(act_account, ids);
        }
        void ChainDatabase::transaction_erase_from_act_balance(const string & act_account, const ActTrxidBalance & act_balance_entry)
        {
            auto iter_ids = my->_act_input_balance_entry.unordered_find(act_account);
            if (iter_ids != my->_act_input_balance_entry.unordered_end())
            {
                auto ids = iter_ids->second;
                ids.erase(act_balance_entry);
                if (!(ids.empty()))
                {
                    my->_act_input_balance_entry.store(act_account, ids);
                }
                ...
                {
                    my->_act_input_balance_entry.remove(act_account);
                }
            }
        }
        ...
        void ChainDatabase::dump_state(const fc::path& path)const
        {
            try {
                ...
                ulog("Dumped ${p}", ("p", next_path));
                next_path = dir / "act_transaction_balance_db.json";
                my->_act_input_balance_entry.export_to_json(next_path);
