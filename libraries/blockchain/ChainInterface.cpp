...
namespace thinkyoung {
    namespace blockchain {
        ...
        {
            try {
                if (name.size() < ACT_BLOCKCHAIN_MIN_NAME_SIZE) return false;
                if (name.size() > ACT_BLOCKCHAIN_MAX_NAME_SIZE) return false;
        ...
        {
            try {
                // if( get_head_block_num() < ACT_V0_5_0_FORK_BLOCK_NUM )
                // return is_valid_symbol_name_v1( symbol );

                FC_ASSERT(symbol != "ACT", "Symbol must not be ACT");

                if (symbol.size() < ACT_BLOCKCHAIN_MIN_SYMBOL_SIZE)
                ...
                if (dots == 0)
                {
                    if (position > ACT_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }
                else //dots == 1 means hierarchial asset name
                {
                    if (position - dot_position - 1 > ACT_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE)
                        FC_ASSERT(false, "Sub-symbol name too large");
                    if (symbol.size() > ACT_BLOCKCHAIN_MAX_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }
        ...
        {
            ShareType pay_per_block = ACT_MAX_DELEGATE_PAY_PER_BLOCK;
        ...
        {
            // if( get_head_block_num() < ACT_V0_4_24_FORK_BLOCK_NUM )
            ...
            const ShareType max_pay_per_two_weeks = max_total_pay_per_two_weeks / ACT_BLOCKCHAIN_NUM_DELEGATES;
        ...
        {
            if (imessage.size() < ACT_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
            {
                return 0;
            }
            ImessageIdType fee_coe = ACT_BLOCKCHAIN_MIN_MESSAGE_FEE_COE;
            ShareType fee = 0;
            fee = fee_coe * (imessage.size() - ACT_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
            return fee;
        }
        ShareType ChainInterface::get_asset_registration_fee(uint8_t symbol_length)const
        {
            ...
            static const ShareType long_symbol_price = 500 * ACT_BLOCKCHAIN_PRECISION; // $10 at $0.02/XTS
        ...
        double ChainInterface::to_pretty_price_double(const Price& price_to_pretty_print)const
        {
            ...
            return fc::variant(string(price_to_pretty_print.ratio * obase_asset->precision / oquote_asset->precision)).as_double() / (ACT_BLOCKCHAIN_MAX_SHARES * 1000);
        }
        ...
        uint64_t ChainInterface::get_required_confirmations()const
        {
            try {
                ...
                return ACT_BLOCKCHAIN_NUM_DELEGATES * 3;
            } FC_CAPTURE_AND_RETHROW()
        }
        ...
        {
            Asset fee(ACT_DEFAULT_TRANSACTION_FEE, 0);
            return fee;
        }
        ...
        {
            Asset fee(ACT_DEFAULT_CONTRACT_MARGIN, 0);
            return fee;
        }
        ...
        {
            return ACT_DEFAULT_TRANSACTION_EXPIRATION_SEC;
        }
        */
            ...
            const Address& to_account_address, SignedTransaction& trx)
        {
            try {
				...
				{
					FC_CAPTURE_AND_THROW(invalid_asset_symbol,("Only ACT Allowed"));
				}
