    ...
    namespace wallet {
        ...
        namespace detail {
            ...
            void WalletImpl::state_changed(const PendingChainStatePtr& state)
            {
                ...
                if (_blockchain->get_head_block_num() < last_unlocked_scanned_number)
                {
                    ...
                    self->set_last_scanned_block_number_for_act(_blockchain->get_head_block_num());
                }
            }
            ...
            void WalletImpl::block_applied(const BlockSummary& summary)
            {
                ...
                if ((t - _blockchain->get_head_block_timestamp()) <= fc::seconds(ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2))
            ...
            */
                ...
                try {
                    ...
                    try
                    {
                        ...
                        while (current_block_num <= head_block_num && count < limit && !_scan_in_progress.canceled())
                        {
                            ...
                            self->set_last_scanned_block_number_for_act(prev_block_num);
                            ++current_block_num;
            ...
            void WalletImpl::upgrade_version()
            {
                ...
                if (current_version > ACT_WALLET_VERSION)
                {
                    FC_THROW_EXCEPTION(unsupported_version, "Wallet version newer than client supports!",
                        ("wallet_version", current_version)("supported_version", ACT_WALLET_VERSION));
                }
                else if (current_version == ACT_WALLET_VERSION)
                {
                    return;
                }
                ...
                try
                {
                    ...
                    {
                        self->set_transaction_fee(Asset(ACT_WALLET_DEFAULT_TRANSACTION_FEE));
                        self->set_transaction_imessage_fee_coe(ACT_BLOCKCHAIN_MIN_MESSAGE_FEE_COE);
                        self->set_transaction_imessage_soft_max_length(ACT_BLOCKCHAIN_MAX_SOFT_MAX_MESSAGE_SIZE);
                    }

                    if (current_version < 106)
                    {
                        self->set_transaction_expiration(ACT_WALLET_DEFAULT_TRANSACTION_EXPIRATION_SEC);
                    }
                    ...
                    {
                        self->set_last_scanned_block_number_for_act(0);
                    }
                    if (_unlocked_upgrade_tasks.empty())
                    {
                        self->set_version(ACT_WALLET_VERSION);
                ...
                const string& brainkey)
            {
                try {
                    ...
                    if (password.size() < ACT_WALLET_MIN_PASSWORD_LENGTH)
             ...
             *  returned will contain no more than ACT_BLOCKCHAIN_MAX_SLATE_SIZE delegates.
             */
            SlateEntry WalletImpl::get_delegate_slate(const VoteStrategy strategy)const
            {
                ...
                {
                    slate_size = std::min<size_t>(ACT_BLOCKCHAIN_MAX_SLATE_SIZE, for_candidates.size());
                }
                else if (strategy == vote_random)
                {
                    slate_size = std::min<size_t>(ACT_BLOCKCHAIN_MAX_SLATE_SIZE / 3, for_candidates.size());
                    slate_size = rand() % (slate_size + 1);
                }
                else if (strategy == vote_recommended && for_candidates.size() < ACT_BLOCKCHAIN_MAX_SLATE_SIZE)
                {
                    ...
                    while (for_candidates.size() < ACT_BLOCKCHAIN_MAX_SLATE_SIZE && recommended_candidate_ranks.size() > 0)
                    {
                        int best_rank = 0;
                ...
                FC_ASSERT(entry.slate.size() <= ACT_BLOCKCHAIN_MAX_SLATE_SIZE);
            ...
            const string& brainkey)
        {
            try {
                ...
                try
                {
                    ...
                    open(wallet_name);
                    unlock(password, ACT_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        void Wallet::create_from_json(const path& filename, const string& wallet_name, const string& passphrase)
        {
            try {
                ...
                try
                {
                    ...
                    close();
                    open(wallet_name);
                    unlock(passphrase, ACT_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        bool Wallet::check_passphrase(const string& passphrase)
        {
            try {
                ...
                if (passphrase.size() < ACT_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
        ...
        void Wallet::change_passphrase(const string & old_passphrase, const string& new_passphrase)
        {
            try {
                if (NOT is_open()) FC_CAPTURE_AND_THROW(wallet_closed);
                if (NOT is_unlocked()) FC_CAPTURE_AND_THROW(wallet_locked);
                if (new_passphrase.size() < ACT_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
            ...
            const variant& private_data)
        {
            try {
                ...
                if (num_accounts_before == 0)
                {
                    ...
                    set_last_scanned_block_number_for_act(0);
                }
        ...
        VoteSummary Wallet::get_vote_status(const string& account_name)
        {
            ...
            for (const auto& item : items)
            {
                ...
                for (const auto& entry : entrys)
                {
                    if (entry.asset_id() != 0)
                        continue;
                    if (entry.slate_id() != my_slate.id() && entry.balance > 1 * ACT_BLOCKCHAIN_PRECISION) // ignore dust
                    ...
                    total_possible += entry.get_spendable_balance(my->_blockchain->now()).amount * ACT_BLOCKCHAIN_MAX_SLATE_SIZE;
                }
            }
        ...
        void Wallet::repair_entrys(const optional<string>& collecting_account_name)
        {
            try {
                ...
                for (const auto& item : items)
                {
                    ...
                    if (name.find(ACT_ADDRESS_PREFIX) != 0)
                        continue;
        ...
        WalletTransactionEntry Wallet::register_contract(const string& owner, const fc::path codefile, const string& asset_symbol, double init_limit, bool is_testing)
        {
            ...
            FC_ASSERT(asset_symbol == ACT_BLOCKCHAIN_SYMBOL, "asset_symbol must be ACT");
        ...
        std::vector<thinkyoung::blockchain::Asset> Wallet::register_contract_testing(const string& owner, const fc::path codefile)
        {
            ...
            auto trans_entry = register_contract(owner,
                codefile,
                ACT_BLOCKCHAIN_SYMBOL,
                (double)asset_for_exec.amount / ACT_BLOCKCHAIN_PRECISION, 
                true
                );
            ...
            const string& act_account)
        {
            try {
                ...
                if (act_account != "")
                {
                    ...
                    trx.act_account = act_account;
                    trx.act_inport_asset = asset_to_transfer;
                }
                // 	  if (memo_message != "")
                // 	  {
            ...
            bool is_testing)
        {
            try {
                ...
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
                FC_ASSERT(amount_to_transfer_symbol == ACT_BLOCKCHAIN_SYMBOL, "Asset symbol should be ACT");
			...
			)
		{
			try
			{
				...
				if (margin_balance_entry->balance != ACT_DEFAULT_CONTRACT_MARGIN)
					FC_CAPTURE_AND_THROW(invalid_margin_amount, (margin_balance_entry->balance));


				Asset margin = Asset(ACT_DEFAULT_CONTRACT_MARGIN);
				Asset fee = get_transaction_fee(0);
				Asset asset_for_exec = my->_blockchain->get_amount(CONTRACT_TESTING_LIMIT_MAX);
			...
			bool is_testing)
        {
            try
			{
                ...
                FC_ASSERT(balance_entry.valid(), "Can not get margin balance!");
                FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not ACT!");
                FC_ASSERT(balance_entry->balance == ACT_DEFAULT_CONTRACT_MARGIN, "invalid margin balance amount");
            ...
            bool sign)
        {
            try {
                ...
                {
#if 0
                    required_fees += Asset(ACT_BLOCKCHAIN_REGISTER_ACCOUNT_FEE, 0);
#endif
                }
            ...
            bool sign)
        {
            try {
                ...
                FC_ASSERT(ACT_BLOCKCHAIN_MAX_SHARES > max_share_supply_in_internal_units);
        ...
        void Wallet::set_transaction_fee(const Asset& fee)
        {
            try {
                ...
                if (fee.amount < ACT_DEFAULT_TRANSACTION_FEE || fee.asset_id != 0)
        ...
        Asset Wallet::get_transaction_imessage_fee(const std::string & imessage)const
        {
            try
            {
                ...
                Asset require_fee;
                    ...
                    if (ACT_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE >= imessage.size())
                    {
                        return require_fee;
                    }
                    auto min_fee_coe = get_transaction_imessage_fee_coe();
                    require_fee.amount = min_fee_coe * (imessage.size() - ACT_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
        ...
        Asset Wallet::get_transaction_fee(const AssetIdType desired_fee_asset_id)const
        {
            try {
                ...
                Asset xts_fee(ACT_WALLET_DEFAULT_TRANSACTION_FEE, 0);
                try
                {
        ...
        void Wallet::set_last_scanned_block_number_for_act(uint32_t block_num)
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
        ...
        uint32_t Wallet::get_last_scanned_block_number_for_act()const
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                try
        ...
        void Wallet::set_transaction_imessage_fee_coe(const ImessageIdType& coe)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (coe < ACT_BLOCKCHAIN_MIN_MESSAGE_FEE_COE)
                    {
        ...
        void Wallet::set_transaction_imessage_soft_max_length(const ImessageLengthIdType& length)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (length < 0 || length > ACT_BLOCKCHAIN_MAX_MESSAGE_SIZE)
                    {
        ...
        void Wallet::set_transaction_expiration(uint32_t secs)
        {
            try {
                ...
                if (secs > ACT_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC)
