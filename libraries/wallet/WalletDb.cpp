    ...
    namespace wallet {
            ...
            public:
                ...
                void load_key_entry(const WalletKeyEntry& key_entry)
                {
                    try {
                        ...
                        // Cache address map
                        self->btc_to_act_address[key_address] = key_address;
						self->btc_to_act_address[Address(PtsAddress(key_entry.public_key, false, 0))] = key_address; // Uncompressed BTC
						self->btc_to_act_address[Address(PtsAddress(key_entry.public_key, true, 0))] = key_address; // Compressed BTC
						self->btc_to_act_address[Address(PtsAddress(key_entry.public_key, false, 56))] = key_address; // Uncompressed PTS
						self->btc_to_act_address[Address(PtsAddress(key_entry.public_key, true, 56))] = key_address; // Compressed PTS
        ...
        void WalletDb::remove_account(const string& accountname)
        {
            try{
                ...
                name_to_account_wallet_entry_index.erase(accRec->name);
                account_id_to_wallet_entry_index.erase(accRec->id);
                btc_to_act_address.erase(accRec->owner_address());
				btc_to_act_address.erase(Address(PtsAddress(accRec->owner_key, false, 0)));
				btc_to_act_address.erase(Address(PtsAddress(accRec->owner_key, true, 0)));
				btc_to_act_address.erase(Address(PtsAddress(accRec->owner_key, false, 56)));
				btc_to_act_address.erase(Address(PtsAddress(accRec->owner_key, true, 56)));
                if (accRec->is_delegate())
                {
					btc_to_act_address.erase(accRec->signing_address());
					btc_to_act_address.erase(Address(PtsAddress(accRec->signing_key(), false, 0)));
					btc_to_act_address.erase(Address(PtsAddress(accRec->signing_key(), true, 0)));
					btc_to_act_address.erase(Address(PtsAddress(accRec->signing_key(), false, 56)));
					btc_to_act_address.erase(Address(PtsAddress(accRec->signing_key(), true, 56)));
                }
				btc_to_act_address.erase(Address(accRec->active_key()));
				btc_to_act_address.erase(Address(PtsAddress(accRec->active_key(), false, 0)));
				btc_to_act_address.erase(Address(PtsAddress(accRec->active_key(), true, 0)));
				btc_to_act_address.erase(Address(PtsAddress(accRec->active_key(), false, 56)));
				btc_to_act_address.erase(Address(PtsAddress(accRec->active_key(), true, 56)));
        ...
        oWalletKeyEntry WalletDb::lookup_key(const Address& derived_address)const
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
				const auto address_map_iter = btc_to_act_address.find(derived_address);
				if (address_map_iter != btc_to_act_address.end())
                {
                        ...
                        if (WalletEntryTypeEnum(entry.type) == key_entry_type)
                        {
                            ...
                            if (key_entry.has_private_key())
                            {
                                ...
                                if (key_entry.public_key != public_key)
                                {
									...
									btc_to_act_address.erase(key_address);
									btc_to_act_address.erase(Address(PtsAddress(key_entry.public_key, false, 0)));
									btc_to_act_address.erase(Address(PtsAddress(key_entry.public_key, true, 0)));
									btc_to_act_address.erase(Address(PtsAddress(key_entry.public_key, false, 56)));
									btc_to_act_address.erase(Address(PtsAddress(key_entry.public_key, true, 56)));
