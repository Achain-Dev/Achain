    ...
    bool bNeedcreate)
{
    ...
    // 
        ...
        {
            if (store_entry && transaction.act_account != "")
            {
                ActTrxidBalance act_entry;
                ...
                for (const auto item : pretty_trx.ledger_entries)
                {
                    ...
                    {
                        act_entry.from_account = item.from_account;
                        break;
                    }
                }
                string strToAccount;
                string strSubAccount;
                self->accountsplit(transaction.act_account, strToAccount, strSubAccount);
                auto blockchain_trx = _blockchain->get_transaction(transaction_id);
                bool bDiffDeop = false;
                if (blockchain_trx.valid())
                {
                    ShareType act_amount = 0;
                    string preowner;
                    int iLoop = 0;
                ...
                //act_entry.from_account = transaction.from_account;
                act_entry.create_time = transaction_entry->created_time;
				act_entry.act_account = transaction.act_account;
				act_entry.block_num = transaction_entry->block_num;
                transaction_entry->trx.act_account = transaction.act_account;
                transaction_entry->trx.act_inport_asset = transaction.act_inport_asset;
				act_entry.trx_id = transaction.id();
                    //act_entry.asset_trx = transaction.act_inport_asset;
						act_entry.asset_trx = Asset(act_amount, 0);
                    {
                        writelock wlock(m_mutex_for_act);
						_blockchain->transaction_insert_to_act_full_entry(strToAccount, act_entry);
						//_blockchain->transaction_insert_to_act_balance(act_entry.act_account, act_entry);
                    }
                }
            }
        }
    ...
    TrxType trx_search_type)const
{
    try {
        ...
        if (!asset_symbol.empty() && asset_symbol != ACT_BLOCKCHAIN_SYMBOL)
        {
            try
            {
...
PrettyTransaction		Wallet::to_pretty_trx(const thinkyoung::blockchain::TransactionEntry& trx_entry, const std::string addr_for_fee) const
{
    ...
    for (const auto& op : trx.operations)
    {
		...
		case update_account_op_type:
		{
            ...
            //所以现在只能简单粗暴得用一个1000ACT来进行划分，只要是大于1000ACT都认为是升级代理
            if (total_fee <= Asset( 1000 * ACT_BLOCKCHAIN_PRECISION ))
            {
...
PrettyContractTransaction		Wallet::to_pretty_contract_trx(const thinkyoung::blockchain::TransactionEntry& trx_entry) const
{
    ...
    if (contract_op_type == thinkyoung::blockchain::OperationTypeEnum::register_contract_op_type)
    {
        ...
        to_contract_ledger_entry.amount = Asset(ACT_DEFAULT_CONTRACT_MARGIN, 0);
        ...
        if (register_success)
        {
            to_contract_ledger_entry.fee = Asset(all_cost - ACT_DEFAULT_CONTRACT_MARGIN, 0);
