    ...
    const string& act_account)
{
    try {
        ...
        if (memo.size() > ACT_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE)
    ...
    const string& act_account)
{
    try {
        ...
        // Don't automatically truncate memos as long as users still depend on them via deposit ops rather than mail
        if (memo.size() > ACT_BLOCKCHAIN_MAX_MEMO_SIZE)
            FC_CAPTURE_AND_THROW(memo_too_long, (memo));
    ...
    )
{
    try {
        ...
        if (market_fee >= 0 || market_fee <= ACT_BLOCKCHAIN_MAX_UIA_MARKET_FEE)
            actual_market_fee = ACT_BLOCKCHAIN_MAX_UIA_MARKET_FEE * market_fee;
