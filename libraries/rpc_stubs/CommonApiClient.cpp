...
namespace thinkyoung {
    namespace rpc_stubs {
        ...
        std::vector<thinkyoung::blockchain::ActTrxidBalance> CommonApiClient::blockchain_get_act_account_balance_entry(uint32_t block_num)
        {
            ilog("received RPC call: blockchain_get_act_account_balance_entry(${block_num})", ("block_num", block_num));
            thinkyoung::api::GlobalApiLogger* glog = thinkyoung::api::GlobalApiLogger::get_instance();
            uint64_t call_id = 0;
            fc::variants args;
            if( glog != NULL )
            {
                ...
                call_id = glog->log_call_started( this, "blockchain_get_act_account_balance_entry", args );
            }
            ...
            struct scope_exit
            {
                fc::time_point start_time;
                scope_exit() : start_time(fc::time_point::now()) {}
                ~scope_exit() { dlog("RPC call blockchain_get_act_account_balance_entry finished in ${time} ms", ("time", (fc::time_point::now() - start_time).count() / 1000)); }
            } execution_time_logger;
            try
            {
                std::vector<thinkyoung::blockchain::ActTrxidBalance> result =             get_impl()->blockchain_get_act_account_balance_entry(block_num);
                if( call_id != 0 )
                    glog->log_call_finished( call_id, this, "blockchain_get_act_account_balance_entry", args, fc::variant(result) );
