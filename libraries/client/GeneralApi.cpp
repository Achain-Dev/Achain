    ...
    namespace client {
        namespace detail {
            ...
            variant_object ClientImpl::get_info()const
            {
                ...
                const auto share_entry = _chain_db->get_asset_entry(ACT_BLOCKCHAIN_SYMBOL);
                if (share_entry.valid())
                    info["blockchain_share_supply"] = share_entry->current_share_supply;

                const auto blocks_left = ACT_BLOCKCHAIN_NUM_DELEGATES - (head_block_num % ACT_BLOCKCHAIN_NUM_DELEGATES);
                ...
                if (head_block_num > 0)
                {
                    ...
                    const auto next_round_timestamp = current_round_timestamp + (blocks_left * ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
