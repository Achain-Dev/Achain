...
namespace thinkyoung {
    namespace client {
        ...
        const string ACT_MESSAGE_MAGIC = "Act Signed Message:\n";
        ...
        program_options::variables_map parse_option_variables(int argc, char** argv)
        {
            ...
            option_config.add_options()
                ...
                ("growl-identifier", program_options::value<std::string>(), "A name displayed in growl messages to identify this ACT_client instance")
                ;
...
#ifdef ACT_TEST_NETWORK
                        //dir_name += "-Test" + std::to_string(ACT_TEST_NETWORK_VERSION);
                        dir_name += "-Test";
#endif
                        return dir_name;
                    };

                    datadir = fc::app_path() / get_os_specific_dir_name(ACT_BLOCKCHAIN_NAME);
                }
                return datadir;
        ...
        namespace detail
        {
            ...
            void ClientImpl::delegate_loop()
            {
                ...
                {
                    set_target_connections(ACT_NET_DELEGATE_DESIRED_CONNECTIONS);
                ...
                if (next_block_time.valid())
                {
                    ...
                    if (*next_block_time <= now)
                    {
                        try
                        {
                            ...
                            FC_ASSERT((now - *next_block_time) < fc::seconds(ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
            ...
            void ClientImpl::start_rebroadcast_pending_loop()
            {
                if (!_rebroadcast_pending_loop_done.valid() || _rebroadcast_pending_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 1.3)),
                    "rebroadcast_pending");
            }
            void ClientImpl::start_fork_update_time_loop()
            {
                if (!_fork_update_loop_done.valid() || _fork_update_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 6)),
                    "rebroadcast_pending");
            }
            ...
            bool ClientImpl::handle_message(const thinkyoung::net::Message& message_to_handle, bool sync_mode)
            {
                ...
                try
                {
                    ...
                    case block_message_type:
                    {
                        ...
                        if (block_message_to_handle.block.block_size() > ACT_BLOCKCHAIN_MAX_BLOCK_SIZE)
                        {
                    ...
                    case trx_message_type:
                    {
                        ...
                        if (trx_message_to_handle.trx.data_size() > ACT_BLOCKCHAIN_MAX_TRX_SIZE)
                        {
            ...
            */
            ...
            void ClientImpl::blocks_too_old_monitor_task()
            {
                ...
                if (_chain_db->get_head_block().timestamp < thinkyoung::blockchain::now() - fc::seconds(ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2) &&
                    !_sync_mode &&
                ...
                if (!_blocks_too_old_monitor_done.canceled())
                    ...
                    fc::time_point::now() + fc::seconds(ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
                    "block_monitor_task");
            }
        ...
        */
...
#ifdef ACT_TEST_NETWORK
            //client_version += "-testnet-" + std::to_string(ACT_TEST_NETWORK_VERSION);
            client_version += "-testnet";
#endif

            fc::mutable_variant_object info;
            info["blockchain_name"] = ACT_BLOCKCHAIN_NAME;
            info["blockchain_description"] = ACT_BLOCKCHAIN_DESCRIPTION;
            info["client_version"] = client_version;
            info["act_revision"] = thinkyoung::utilities::git_revision_sha;
            info["act_revision_age"] = fc::get_approximate_relative_time_string(fc::time_point_sec(thinkyoung::utilities::git_revision_unix_timestamp));
...
#if defined(ACT_CLIENT_JENKINS_BUILD_NUMBER)
            info["jenkins_build_number"] = ACT_CLIENT_JENKINS_BUILD_NUMBER;
#endif
#if defined(ACT_CLIENT_JENKINS_BUILD_URL)
            info["jenkins_build_url"] = ACT_CLIENT_JENKINS_BUILD_URL;
#endif
        ...
        void Client::configure_from_command_line(int argc, char** argv)
        {
            ...
            if (option_variables.count("growl") || my->_config.growl_notify_endpoint)
            {
                ...
                std::string act_instance_identifier = "Act";
                if (option_variables.count("growl-identifier"))
                    act_instance_identifier = option_variables["growl-identifier"].as<std::string>();
                else if (my->_config.growl_act_client_identifier)
                    act_instance_identifier = *my->_config.growl_act_client_identifier;
                my->_notifier = std::make_shared<ActGntpNotifier>(host_to_notify, port_to_notify, act_instance_identifier, growl_password);
