    ...
    namespace client {
        namespace detail {
            ...
            void detail::ClientImpl::wallet_create(const string& wallet_name, const string& password, const string& brain_key)
            {
                ...
                if (brain_key.size() && brain_key.size() < ACT_WALLET_MIN_BRAINKEY_LENGTH) FC_CAPTURE_AND_THROW(brain_key_too_short);
                if (password.size() < ACT_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
            ...
            */
            ...
            void ClientImpl::wallet_set_transaction_imessage_fee_coe(const string& fee_coe)
            {
                ...
                try
                {
                    ...
                    if (ipos != string::npos)
                    {
                        ...
                        FC_ASSERT((precision_input <= ACT_BLOCKCHAIN_PRECISION), "Precision is not correct");
                    }
                    double dFee = std::stod(fee_coe);
                    _wallet->set_transaction_imessage_fee_coe(static_cast<int64_t>(floor(dFee * ACT_BLOCKCHAIN_PRECISION + 0.5)));
                }
            ...
            double ClientImpl::wallet_get_transaction_imessage_fee_coe()
            {
                ...
                return ((double)fee_coe) / ACT_BLOCKCHAIN_PRECISION;
            }
            ...
            Asset ClientImpl::wallet_get_transaction_fee(const string& fee_symbol)
            {
                ...
                if (fee_symbol.empty())
                    return _wallet->get_transaction_fee(_chain_db->get_asset_id(ACT_BLOCKCHAIN_SYMBOL));
