    ...
    namespace client {
        namespace detail {
            ...
            vector<AccountEntry> ClientImpl::blockchain_list_active_delegates(uint32_t first, uint32_t count)const
            {
                ...
                if (first > 0) --first;
                FC_ASSERT(first < ACT_BLOCKCHAIN_NUM_DELEGATES, "Only the first 99 delegates are active");
                FC_ASSERT(first + count <= ACT_BLOCKCHAIN_NUM_DELEGATES, "Only the first 99 delegates are active");
                vector<AccountIdType> all_delegate_ids = _chain_db->get_active_delegates();
                FC_ASSERT(all_delegate_ids.size() == ACT_BLOCKCHAIN_NUM_DELEGATES, "The num of active delegates should equal to ACT_BLOCKCHAIN_NUM_DELEGATES");
            ...
            vector<string> ClientImpl::blockchain_list_missing_block_delegates(uint32_t block_num)
            {
                ...
                timestamp += ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                while (timestamp != this_block.timestamp)
                    // if time interval between prev_block and this block greater than ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
            ...
            vector<ActTrxidBalance> ClientImpl::blockchain_get_act_account_balance_entry(const uint32_t block_num)
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");

                uint32_t last_scan_num = _wallet->get_last_scanned_block_number_for_act();
                auto results = _chain_db->fetch_act_full_entry(block_num, last_scan_num);
            ...
            oAccountEntry detail::ClientImpl::blockchain_get_account(const string& account)const
            {
                ...
                try
                {
                    ...
                    else if (account.substr(0, string(ACT_ADDRESS_PREFIX).size()) == ACT_ADDRESS_PREFIX) {
			...
			std::vector<fc::string> ClientImpl::blockchain_list_pub_all_address(const std::string& pub_key) const
			{
				...
				try
				{
					...
					if (fc::string::npos != pub_key.find(ACT_ADDRESS_PREFIX))
					{
            ...
            variant_object ClientImpl::blockchain_get_info()const
            {
                ...
                info["name"] = ACT_BLOCKCHAIN_NAME;
                info["symbol"] = ACT_BLOCKCHAIN_SYMBOL;
                info["address_prefix"] = ACT_ADDRESS_PREFIX;
                info["db_version"] = ACT_BLOCKCHAIN_DATABASE_VERSION;
                info["genesis_timestamp"] = _chain_db->get_genesis_timestamp();

                info["block_interval"] = ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                info["delegate_num"] = ACT_BLOCKCHAIN_NUM_DELEGATES;
                ...
                info["name_size_max"] = ACT_BLOCKCHAIN_MAX_NAME_SIZE;
                info["memo_size_max"] = ACT_BLOCKCHAIN_MAX_MESSAGE_SIZE;
                info["data_size_max"] = ACT_BLOCKCHAIN_MAX_NAME_DATA_SIZE;

                info["symbol_size_max"] = ACT_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE;
                info["symbol_size_min"] = ACT_BLOCKCHAIN_MIN_SYMBOL_SIZE;
                info["asset_shares_max"] = ACT_BLOCKCHAIN_MAX_SHARES;
                info["short_symbol_asset_reg_fee"] = _chain_db->get_asset_registration_fee(ACT_BLOCKCHAIN_MIN_SYMBOL_SIZE);
                info["long_symbol_asset_reg_fee"] = _chain_db->get_asset_registration_fee(ACT_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE);

                info["statistics_enabled"] = _chain_db->get_statistics_enabled();

                info["relay_fee"] = _chain_db->get_relay_fee();
                info["max_pending_queue_size"] = ACT_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
                info["max_trx_per_second"] = ACT_BLOCKCHAIN_MAX_TRX_PER_SECOND;
            ...
            thinkyoung::blockchain::BlockchainSecurityState ClientImpl::blockchain_get_security_state()const
            {
                ...
                state.estimated_confirmation_seconds = (uint32_t)(required_confirmations * ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
                ...
                else if (required_confirmations <= ACT_BLOCKCHAIN_NUM_DELEGATES / 2
                    && participation_rate > 80)
                {
                    state.alert_level = thinkyoung::blockchain::BlockchainSecurityState::green;
                }
                else if (required_confirmations > ACT_BLOCKCHAIN_NUM_DELEGATES
            ...
            bool ClientImpl::blockchain_is_synced() const
            {
                ...
                return (blockchain::now() - _chain_db->get_head_block().timestamp) < fc::seconds(ACT_BLOCKCHAIN_NUM_DELEGATES * ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
            }
