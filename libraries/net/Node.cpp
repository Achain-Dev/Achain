...
#define MAXIMUM_NUMBER_OF_BLOCKS_TO_PREFETCH (10 * MAXIMUM_NUMBER_OF_BLOCKS_TO_HANDLE_AT_ONE_TIME)
            ...
            NodeImpl::NodeImpl(const std::string& user_agent) :
                ...
                _desired_number_of_connections(ACT_NET_DEFAULT_DESIRED_CONNECTIONS),
                _maximum_number_of_connections(ACT_NET_DEFAULT_MAX_CONNECTIONS),
                _peer_connection_retry_timeout(ACT_NET_DEFAULT_PEER_CONNECTION_RETRY_TIME),
                _peer_inactivity_timeout(ACT_NET_PEER_HANDSHAKE_INACTIVITY_TIMEOUT),
                ...
                _maximum_blocks_per_peer_during_syncing(ACT_NET_MAX_BLOCKS_PER_PEER_DURING_SYNCING),
                _sync_item_type(0)
            {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            if (is_wanting_new_connections() || !_add_once_node_list.empty())
                            {
                                ...
                                _retrigger_connect_loop_promise->wait_until(fc::time_point::now() + fc::seconds(ACT_PEER_DATABASE_RETRY_DELAY));
                            }
                            else
                            {
                    ...
                    for (auto item_iter = _items_to_fetch.begin(); item_iter != _items_to_fetch.end();)
                    {
                        ...
                        else
                        {
                            ...
                            for (auto peer_iter = items_by_peer.get<requested_item_count_index>().begin(); peer_iter != items_by_peer.get<requested_item_count_index>().end(); ++peer_iter)
                            {
                                ...
                                if (peer_iter->item_ids.size() < ACT_NET_MAX_ITEMS_PER_PEER_DURING_NORMAL_OPERATION &&
            ...
            void NodeImpl::terminate_inactive_connections_loop()
            {
                ...
                uint32_t active_disconnect_timeout = std::max<uint32_t>(5 * ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC / 2, 30);
                uint32_t active_send_keepalive_timeount = std::max<uint32_t>(active_disconnect_timeout / 2, 11);
                uint32_t active_ignored_request_timeount = std::max<uint32_t>(ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC / 4, 10);
                ...
                fc::time_point closing_disconnect_threshold = fc::time_point::now() - fc::seconds(ACT_NET_PEER_DISCONNECT_TIMEOUT);
                for (const PeerConnectionPtr& closing_peer : _closing_connections)
                    ...
                    {
                        // we asked this peer to close their connectoin to us at least ACT_NET_PEER_DISCONNECT_TIMEOUT
                ...
                if (!_node_is_shutting_down && !_terminate_inactive_connections_loop_done.canceled())
                    ...
                    fc::time_point::now() + fc::seconds(ACT_NET_PEER_HANDSHAKE_INACTIVITY_TIMEOUT / 2),
            ...
            fc::variant_object NodeImpl::generate_hello_user_data()
            {
                ...
                user_data["delegate_num"] = ACT_BLOCKCHAIN_NUM_DELEGATES;
            ...
            void NodeImpl::on_hello_message(PeerConnection* originating_peer, const HelloMessage& hello_message_received)
            {
                ...
                if (originating_peer->their_state == PeerConnection::TheirConnectionState::just_connected)
                {
                    ...
                    if (delegate_num != ACT_BLOCKCHAIN_NUM_DELEGATES)
                    {
                        wlog("Invalid client");
                ...
                const BlockchainItemIdsInventoryMessage& blockchain_item_ids_inventory_message_received)
            {
                ...
                if (originating_peer->item_ids_requested_from_peer)
                {
                    ...
                    fc::time_point_sec minimum_time_of_last_offered_block =
                        ...
                        originating_peer->number_of_unfetched_item_ids * ACT_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    if (minimum_time_of_last_offered_block > _delegate->get_blockchain_now() + ACT_NET_FUTURE_SYNC_BLOCKS_GRACE_PERIOD_SEC)
                    {
                    ...
                    if (blockchain_item_ids_inventory_message_received.total_remaining_item_count != 0)
                    {
                        ...
                        if (originating_peer->ids_of_items_to_get.size() > ACT_NET_MIN_BLOCK_IDS_TO_PREFETCH)
                        {
                ...
                const MessageHashType& message_hash)
            {
                ...
                if (block_message_to_process.block.block_size() > ACT_BLOCKCHAIN_MAX_BLOCK_SIZE)
                {
                    ...
                    if (sync_item_iter != originating_peer->sync_items_requested_from_peer.end())
                    {
                        ...
                        if (originating_peer->idle())
                        {
                            ...
                            if (originating_peer->number_of_unfetched_item_ids > 0 &&
                                originating_peer->ids_of_items_to_get.size() < ACT_NET_MIN_BLOCK_IDS_TO_PREFETCH)
                                fetch_next_batch_of_item_ids_from_peer(originating_peer);
                            else
                                trigger_fetch_sync_items_loop();
                        }
                ...
                else
                {
                    ...
                    catch (const insufficient_relay_fee&)
                    {
                        ...
                        if (message_to_process.msg_type == thinkyoung::client::trx_message_type)
                            originating_peer->transaction_fetching_inhibited_until = fc::time_point::now() + fc::seconds(ACT_NET_INSUFFICIENT_RELAY_FEE_PENALTY_SEC);
                        return;
                    }
                    catch (const fc::canceled_exception&)
                    {
...
#ifdef ACT_TEST_NETWORK
                    //uint32_t port = ACT_NET_TEST_P2P_PORT + ACT_TEST_NETWORK_VERSION;
                    uint32_t port = ACT_NET_TEST_P2P_PORT;
#else
                    uint32_t port = ACT_NET_DEFAULT_P2P_PORT;
#endif
