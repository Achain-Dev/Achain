    ...
    namespace lua {
        namespace api {
            ...
            */
            ...
            bool GluaChainApi::check_act_address(lua_State *L, const char* act_address) {
                string address(act_address);
                size_t first = address.find_first_of(ALP_ADDRESS_PREFIX);
                
                if (first != std::string::npos&&first == 0) {
                    string strToAccount;
                    string strSubAccount;
                    thinkyoung::wallet::Wallet::accountsplit(address, strToAccount, strSubAccount);
