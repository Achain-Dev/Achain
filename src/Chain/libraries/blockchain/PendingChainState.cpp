...
namespace thinkyoung {
    namespace blockchain {
        ...
        oContractValue PendingChainState::contract_lookup_value_by_valueid(const ContractValueIdType& id) const {
            const auto iter = _value_id_to_storage.find(id);
            
            if (iter != _value_id_to_storage.end()) return iter->second;
            
            const ChainInterfacePtr prev_state = _prev_state.lock();
            
            if (!prev_state) return oContractValue();
            
            const oContractValue entry = prev_state->lookup<ContractValueEntry>(id);
            
            //TODO : find _contract_id_remove
            if (entry.valid() && _value_id_remove.count(id) == 0) return entry;
