    ...
    namespace blockchain {
        ...
        TransactionEvaluationStatePtr ChainDatabase::evaluate_transaction(const SignedTransaction& trx,
            ...
            try {
                ...
                //to check ! evaluate_transaction,
                //if delegates, run the evaluate; if not then skip
                trx_eval_state->skipexec = !generating_block;
                
                //if ordinary node(not delegate) open the VM, then run the evaluate
                if (trx_eval_state->skipexec)
                    trx_eval_state->skipexec = !get_node_vm_enabled();
                    
                trx_eval_state->throw_exec_exception = throw_exec_exception;
                
                //transaction of local wallet run vm at the first validation
