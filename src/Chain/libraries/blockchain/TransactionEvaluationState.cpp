...
namespace thinkyoung {
    namespace blockchain {
        ...
        TransactionEvaluationState::TransactionEvaluationState(PendingChainState* current_state)
            ...
            , trx_type(trx_type_undefined)   //formysql
        ...
        */
        ...
        //for mysql
        void TransactionEvaluationState::set_trx_type(){
            op_count = operations.size();

            bool has_withdraw_op_type = false;
            bool has_deposit_op_type = false;
            bool has_withdraw_pay_op_type = false;
            bool has_register_account_op_type = false;
            bool has_update_account_op_type = false;
            bool has_create_asset_op_type = false;
            bool has_update_asset_op_type = false;
            bool has_register_contract_op_type = false;
            bool has_upgrade_contract_op_type = false;
            bool has_destroy_contract_op_type = false;
            bool has_transfer_contract_op_type = false;
            bool has_call_contract_op_type = false;
            bool has_transaction_op_type = false;
            bool has_contract_info_op_type = false;
            bool has_on_call_success_op_type = false;
            bool has_on_destroy_op_type = false;
            bool has_on_upgrade_op_type = false;
            //Operation op(withdraw_op_type);
            for (auto op : operations)
            {
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::withdraw_op_type)
                {
                    has_withdraw_op_type = true;
                    continue;
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::deposit_op_type)
                {
                    has_deposit_op_type = true;
                    continue;
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::withdraw_pay_op_type)
                {
                    has_withdraw_pay_op_type = true;
                    break;      //one op is enough to decide the trx_type
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::register_account_op_type)
                {
                    has_register_account_op_type = true;
                    break;      //one op is enough to decide the trx_type
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::update_account_op_type)
                {
                    has_update_account_op_type = true;
                    break;      //one op is enough to decide the trx_type
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::create_asset_op_type)
                {
                    has_create_asset_op_type = true;
                    break;      //one op is enough to decide the trx_type
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::register_contract_op_type)
                {
                    has_register_contract_op_type = true;
                    break;      //one op is enough to decide the trx_type
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::upgrade_contract_op_type)
                {
                    has_upgrade_contract_op_type = true;
                    break;      //one op is enough to decide the trx_type
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::destroy_contract_op_type)
                {
                    has_destroy_contract_op_type = true;
                    break;      //one op is enough to decide the trx_type
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::transfer_contract_op_type)
                {
                ...
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::transaction_op_type)
                {
                    has_transaction_op_type = true;
                    continue;      //one op is enough to decide the trx_type
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::contract_info_op_type)
                {
                    has_contract_info_op_type = true;
                    continue;      //one op is enough to decide the trx_type
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::on_call_success_op_type)
                {
                    has_on_call_success_op_type = true;
                    continue;      //one op is enough to decide the trx_type
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::on_upgrade_op_type)
                {
                    has_on_upgrade_op_type = true;
                    continue;      //one op is enough to decide the trx_type
                }
                if (op.type == thinkyoung::blockchain::OperationTypeEnum::on_destroy_op_type)
                {
                    has_on_destroy_op_type = true;
                    continue;      //one op is enough to decide the trx_type
                }
            }


            if (has_withdraw_op_type&&has_deposit_op_type)
            {
                trx_type = trx_type_asset_transfer;
            }
            else if (has_withdraw_pay_op_type)
            {
                trx_type = trx_type_agent_get_pay;
            }
            else if (has_register_account_op_type)
            {
                trx_type = trx_type_register_account;
            }
            else if (has_update_account_op_type)
            {
                trx_type = trx_type_update_account;
            }
            else if (has_create_asset_op_type)
            {
                trx_type = trx_type_create_asset;
            }
            else if (has_update_asset_op_type)
            {
                trx_type = trx_type_update_asset;
            }
            else if (has_register_contract_op_type)
            {
                trx_type = trx_type_register_contract;
            }
            else if (has_upgrade_contract_op_type)
            {
                trx_type = trx_type_upgrade_contract;
            }
            else if (has_destroy_contract_op_type)
            {
                trx_type = trx_type_destroy_contract;
            }
            else if (has_transfer_contract_op_type)
            {
                trx_type = trx_type_contract_recharge;
            }
            else if (has_call_contract_op_type)
            {
                if (contract_method == "transfer_to")
                {
                    trx_type = trx_type_contract_transfer;
                }
                else{
                    trx_type = trx_type_call_contract;
                }

            }
            else if (has_transaction_op_type&&has_contract_info_op_type)
            {
                trx_type = trx_type_register_contract_result;
            }
            else if (has_transaction_op_type&&has_on_call_success_op_type)
            {
                trx_type = trx_type_call_contract_result;
            }
            else if (has_transaction_op_type&&has_on_upgrade_op_type)
            {
                trx_type = trx_type_upgrade_contract_result;
            }
            else if (has_transaction_op_type&&has_on_destroy_op_type)
            {
                trx_type = trx_type_destroy_contract_result;
            }
            else
            {
