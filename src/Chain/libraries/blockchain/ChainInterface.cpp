    ...
    namespace blockchain {
        ...
        bool ChainInterface::is_valid_symbol_name(const string& symbol)const {
            try {
                ...
                FC_ASSERT(symbol != "ABTC", "Symbol must not be ABTC");
                ...
                if (symbol.find("ABTC") == 0)
                    FC_ASSERT(false, "Symbol names cannot be prefixed with ABTC");
        ...
        SignedTransaction ChainInterface::transfer_asset_from_contract(
            ...
            try {
                ...
                if (!asset_rec.valid() || asset_rec->id != 0) {
                    FC_CAPTURE_AND_THROW(invalid_asset_symbol, ("Only ABTC Allowed"));
                }
